...
//				showDialog,选择是 捡漏 ，准点抢
	...
	private String[] menus = new String[]{"取消", "准点抢", "捡漏"};
	//弹出单选框
	...
	private void firstBook(){
		...
		int result= random.nextInt(200) + 350;// 返回[0,10)集合中的整数，注意不包括10
	...
	private void endBook(){
		java.util.Random random=new java.util.Random();
		int result= random.nextInt(2000) + 550;// 返回[0,10)集合中的整数，注意不包括10
		mHandler.postDelayed(new Runnable() {
			@Override
			public void run() {
				Calendar instance = Calendar.getInstance();
